@php
    if (Gate::allows('isBendahara')) {
        $jumlah_notif = App\Models\Notification::where('is_read', false)->pluck('jurusan')->count();
    } else {
        $jumlah_notif = App\Models\Notification::where('is_read', false)
...
<li class="nav-item dropdown">
    <a class="nav-link count-indicator position-relative" id="notificationDropdown" href="#" data-bs-toggle="dropdown"
        aria-expanded="false">
        <i class="bi bi-bell-fill fs-4 {{ $jumlah_notif ? 'text-warning animate__animated animate__jello animate__infinite animate__slow' : 'text-dark' }}"
